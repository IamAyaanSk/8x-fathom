...
type MeetingAssistantContext = 'meeting-detail' | 'library'
...
function buildMeetingAssistantInstructions({
  ...
  const contextLine =
    context === 'library'
      ? 'The user is on their meeting library home page. Call searchAllMeetBase before any factual answer — search across all of their processed meetings.'
      : `The user is viewing the meeting titled "${meetingTitle}". Call searchAllMeetBase before any factual answer — search across all of their processed meetings, not just this call.`
  ...
  return [
    ...
    '- Greetings are allowed. For "What can you do?", explain in one short paragraph that you search their processed calls and answer from transcripts.',
    ...
    '- If search returns no snippets, say briefly that nothing relevant was found in their meetings.',
...
export type { MeetingAssistantContext }
