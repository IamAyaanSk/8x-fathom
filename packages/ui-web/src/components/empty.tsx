  ...
  'mb-2 flex shrink-0 items-center justify-center [&_svg]:pointer-events-none [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        icon: "flex size-12 shrink-0 items-center justify-center rounded-2xl border border-border/60 bg-background text-foreground shadow-xs [&_svg:not([class*='size-'])]:size-6"
      }
    },
    defaultVariants: {
      variant: 'default'
...
function EmptyContent({ className, ...props }: ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex w-full min-w-0 max-w-sm flex-col items-center gap-4 text-center text-sm text-balance',
        className
      )}
      {...props}
    />
